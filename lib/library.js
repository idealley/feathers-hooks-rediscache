...
function before(options) {
  ...
  return function (hook) {
    ...
    return new Promise(resolve => {
      ...
      const path = (0, _path.parsePath)(hook, options);
  ...
  return function (hook) {
    ...
    return new Promise(resolve => {
      if (!hook.result.cache.cached) {
        ...
        const path = hook.params.cacheKey || (0, _path.parsePath)(hook, options); // adding a cache object
...
function redisClient() {
  ...
  const redisOptions = Object.assign({}, this.get('redis'), {
    retry_strategy: function (options) {
      ...
      if (cacheOptions.env !== 'test') {
        console.log(`${_chalk.default.yellow('[redis]')} not connected`);
      }
  ...
  const client = _redis.default.createClient(redisOptions);
...
function routes(app) {
  ...
  router.get('/clear/single/*', (req, res) => {
    let target = decodeURIComponent(req.params[0]); // Formated options following ?
